...
5. 通过ReadView可知，对于未提交的版本对于当前事务时不可见的。

6. 在Mysql中，READ COMMITTED和REPEATABLE READ隔离级别的的一个非常大的区别就是它们生成ReadView的时机不同。
    1. 读已提交是每次读取数据（select）都生成一个ReadView；
    因此，当前事务在第二次读的时候，若之前存在一个事务修改数据并提交；则第二次读时生成的ReadView中已经提交事务id被移除，已提交事务的版本则对当前事务可见，造成不可重复读。

    2. 可重复读是在第一次读取数据时生成一个ReadView，在事务后面操作中ReadView保持不变。因此，当前事务在第二次读的时候，若之前存在一个事务修改数据并提交；则第二次读时ReadView还是第一个读的ReadView，已提交事务的版本则对当前事务还是不可见，不会产生不可重复读。
...
快照读是简单的select语句，通过MVCC和undo log读取历史版本数据；
...
若要完全避免，需要手动加锁，以当前读next-key方式读取记录，next-key是行记录锁Record和Gap锁的结合，Record锁能够锁住当前记录，Gap锁锁住当前记录与前一个记录间主键的间隙，避免其他线程在间隙中插入数据。
因此，在当前读时，next-key锁住当前记录的主键间隙，使得其他线程无法插入记录，只能等待当前事务执行完毕，从而完全避免幻读现象。
